...
function guardarInfo(){  //Función para settiar y crear el objecto alumno con sus valores del formulario.
    ...
    let idGrado=document.querySelector("#id").value;   //input del nombre
    let nombreGrado=document.querySelector("#name_grade").value;  //input del apellido
    let subjectGrado=document.querySelector("#fk").value;
    grado = {   // objeto alumno
        pk_id: idGrado,   //se le asigna valor a la clave pk_LRN
        name_grade: nombreGrado,        //se le asigna valor a la clave name
        fk_subject : subjectGrado
    };

    setGrado(grado);   //llamada a la función setAlumno donde se llama a la API create.
      window.location.reload();   //recarga el documento actual
    
}
 
async function setGrado(grado){   // Función asincrona guardar Alumno a la API con parametro de entrada alumno que es un obj
/*  let alumnos =  getAlumno() || [];
 ...
 try{ 
   url_petition = await (await fetch(URL_API + "/grade/create",{   //se le asigna a la funcion url_petition la función para guardar alumno
      method: 'POST',   //se indica el metodo que se utilizara
      body:JSON.stringify(grado)   //agregar body del objecto alumno para volverlo string.
  ...
  return url_petition;          // retornamos url_petition
   ...
   alert("Hubo un error en setGrado " + e);        //mandamos una alerta al usuario 
   return [];                        
 }
}

async function getGrado(){
   ...
   try{
     url_petition= await (await fetch(URL_API + "/grade",{    //se le asigna a la funcion url_petition la función para guardar alumno
    ...
    return url_petition.data;                    // retornamos url_petitio
     ...
     alert("Hubo un error en getGrado" + e);   // enviamos una alerta con el error
     return [];
   }
...
let tablaDeGrado = document.querySelector(".tablaDeAlumnos");   //seleccionamos la parte del documento donde esta la clase tablaDeAlumnos
let cuerpoTabla = document.createElement("tbody");                // creamos un nuevo elemento tbody
getGrado().then(function(grados){                               // ------------------------
    grados.forEach(grado => {                                   // iteramos por cada alumno de la variable alumnos 
        ...
        botonEliminar.setAttribute("data-id", grado.pk_id);   // agregamos el atributo data-matricula al boton
        botonEliminar.addEventListener("click",function(e){
          console.log("Se hizo click");
          let id = e.target.getAttribute("data-id"); 
            deleteGrado(id).then(function(url_petition){
...
/*     function eliminarAlumno(matricula){
  ...
  async function deleteGrado(id){         // se crea la funcion asincrona deleteAlumno
    let url_petition;    //se crea una variable url_petition

    try{ 
      url_petition = await (await fetch(URL_API + "/grade/delete/" + id,{   //se le asigna a la funcion url_petition la función para guardar alumno
         method: 'POST',   //se indica el metodo que se utilizara
     ...
     return url_petition;         // retornamos el valor de url_petition
      ...
      alert("Hubo un error en deleteGrado " + e);   //enviamos una alerta al usuario de que hubo un error
    ...
    window.addEventListener('DOMContentLoaded', (event) => {             //se agrega un evento
        ...
        if(section){                                                     //condicional if seccion == tru
            if(section.getAttribute("data-section") === 'lista' ){       //  condicional if atributo data-section es igual a lista    
                tablaDeGrado.appendChild(cuerpoTabla);                 //se agrega cuerpoTabla a tablaDeAlumnos
